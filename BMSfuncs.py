...
def nnGrad(nn_params,input_layer_size,hidden_layer_size,num_labels,X,y,lamb):
    ...
    for t in range(X.shape[0]):
        ...
        MtoA = np.squeeze(np.asarray(X[t,:]))
        a_1 = np.matrix(np.r_[np.ones(1),MtoA]).T
...
def nnCostFunction(nn_params,input_layer_size,hidden_layer_size,num_labels,X,y,lamb):
    ...
    for t in range(X.shape[0]):
        ...
        z_2 = np.dot(Theta1,a_1)
        a_2 = np.matrix(np.r_[np.ones((1,1)),sigmoid(z_2)])
#        print('the shape of a_2 is: ', a_2.shape)
        z_3 = np.dot(Theta2,a_2)
        a_3 = sigmoid(z_3)
#        print('the shape of a_3 is: ', a_3.shape)

        # 2
#        print('the shape of y[t,0:24].T is: ', np.matrix(y[t,0:24]).T.shape)
        delta3 = a_3-np.matrix(y[t,:]).T
#        print('the shape of delta3 is: ', delta3.shape)

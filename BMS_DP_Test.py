...
print("Loading Data ...\n")
DataTrain = pd.read_csv('Test_u94Q5KV.csv') 
...
Data_Test_Orginal = np.hstack((Item_identifier, Item_weight, Item_fat_content, Item_visibility, 
                      Item_type, Item_mrp, Outlet_identifier, Outlet_establishment_year, 
                      Outlet_location_type, Outlet_type))
print(Data_Test_Orginal)
import csv
b = open('TestDataOrginal.csv', 'a') 
a = csv.writer(b)
a.writerows(Data_Test_Orginal)  
b.close

DTOsize = np.shape(Data_Test_Orginal)
print(DTOsize)
RowSize = DTOsize[0]
...
Data_Test_Normalized = np.zeros((RowSize,ColumnSize))
for j in range(ColumnSize):
    maxValue = max(Data_Test_Orginal[:,j])
    minValue = min(Data_Test_Orginal[:,j])
    for i in range(RowSize):
        Data_Test_Normalized[i,j] = (Data_Test_Orginal[i,j]-minValue)/(maxValue-minValue)
print('All the Normalized Test Data:')
print(Data_Test_Normalized)
b = open('TestDataNormalized.csv', 'a') 
a = csv.writer(b)
a.writerows(Data_Test_Normalized)  
b.close
